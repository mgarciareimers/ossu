...
# http://www.py4e.com/code3/geoxml.py. The program will prompt for a URL, read the XML 
...
# Actual data: http://py4e-data.dr-chuck.net/comments_1450813.xml (Sum ends with 38)
#
...
try:
    xml = urlopen(url, context=context).read()
...
for comment in data.findall('comments/comment'):
    try:
